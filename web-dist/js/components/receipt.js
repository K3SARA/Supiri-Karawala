...
<div class="bill-container">
  ...
  async generateTextSummary(sale, saleItems) {
    const settings = await DB.getAllSettings();
    const shopName = settings.shopName || 'Supiri Karawala';
    const shopPhone = settings.shopPhone || '';
    const customer = sale.customerId ? await DB.getCustomer(sale.customerId) : null;
    ...
    const receiptItems = await this.getReceiptItems(sale, saleItems);
    const itemLines = receiptItems.map((item, idx) => {
      const qty = item.quantity || 0;
      const price = item.price || 0;
      const total = item.total ?? (qty * price - (item.discount || 0));
      const gpp = item.gramsPerPacket || 0;
      const qtyLabel = gpp > 0 ? `${qty} pkts (${gpp}g/pkt)` : `x${qty}`;
      return `${idx + 1}. ${item.name || 'Item'} ${qtyLabel}  LKR ${Utils.currencyPlain(total)}`;
    }).join('\n');
    ...
    return [
      ...
      '',
      '---',
      `Subtotal: LKR ${Utils.currencyPlain(subtotal)}`,
      discount > 0 ? `Discount: -LKR ${Utils.currencyPlain(discount)}` : '',
      tax > 0 ? `Tax: LKR ${Utils.currencyPlain(tax)}` : '',
      `*Total: LKR ${Utils.currencyPlain(total)}*`,
      `Payment: ${this.paymentLabel(sale.paymentMethod)}`,
      '---',
      '',
  ...
  async _openUrl(url) {
    try {
      if (window.SupiriKarawala?.openExternal) {
        await window.SupiriKarawala.openExternal(url);
      } else {
  ...
  async shareWhatsApp(sale, saleItems, phone = '') {
    const text = await this.generateTextSummary(sale, saleItems);
  ...
  async shareViaSMS(sale, saleItems, phone = '') {
    const text = await this.generateTextSummary(sale, saleItems);
    const clean = (phone || '').replace(/[^\d]/g, '');
    const url = clean
  ...
  async showShareModal(sale, saleItems) {
    const customer = sale.customerId ? await DB.getCustomer(sale.customerId) : null;
    ...
    Modal.show({
      ...
      content: `
        <div class="form-group" style="margin-bottom:14px">
          ...
          <input type="tel" class="form-input" id="sharePhoneInput"
            placeholder="94771234567"
            value="${Utils.escapeHtml(rawPhone)}"
            style="font-size:16px">
          <div style="font-size:11px;color:var(--text-secondary);margin-top:4px">Sri Lanka: 94 + number &nbsp;•&nbsp; e.g. 94771234567</div>
        </div>
        <div style="display:flex;gap:8px">
          <button class="btn btn-success" id="shareWaBtn" style="flex:1;font-size:14px">💬 WhatsApp</button>
          <button class="btn btn-outline" id="shareSmsBtn" style="flex:1;font-size:14px">📱 SMS</button>
        </div>
      `,
      footer: `<button class="btn btn-outline" onclick="Modal.close()">Close</button>`
    });
    ...
    document.getElementById('shareWaBtn').addEventListener('click', async () => {
      ...
      await Receipt.shareWhatsApp(sale, saleItems, phone);
    });
